...
const overlayStyle = { position: 'fixed', inset: 0, background: 'rgba(15,23,42,.42)', zIndex: 9999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' };
const panelStyle = { width: '100%', maxWidth: 520, maxHeight: '88dvh', overflow: 'auto', background: '#fff', borderRadius: '22px 22px 0 0', padding: '16px 16px calc(env(safe-area-inset-bottom, 0px) + 16px)', boxShadow: '0 -18px 44px rgba(15,23,42,.22)' };
      ...
      <div style={panelStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#667085', marginTop: 2 }}>{cfg.mode === 'fourball' ? '포볼 히든팀' : '개인 1대1 지목'} · {cfg.revealed ? '공개' : '비공개'}</div>
          </div>
        ...
        {cfg.mode === 'fourball' ? (
          <div style={{ display: 'grid', gap: 8 }}>
            {!teamRows.length && <div style={{ color: '#999', fontSize: 13, border: '1px dashed #d7dfec', borderRadius: 12, padding: 12 }}>아직 포볼팀이 배정되지 않았습니다.</div>}
            {teamRows.map((row, idx) => (
              <div key={row.key} style={{ border: '1px solid #e5eaf2', borderRadius: 12, padding: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  <b>{idx + 1}. {row.label}</b>
                  <b style={{ color: '#1d4ed8' }}>{fmt(row.value)}</b>
                </div>
                <div style={{ marginTop: 4, fontSize: 12, color: '#667085' }}>G합 {fmt(row.handicapSum)}</div>
              </div>
            ))}
          </div>
        ) : (
