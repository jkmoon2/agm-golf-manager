...
const boxStyle = { border: '1px solid #e5eaf2', borderRadius: 14, padding: 12, background: '#fbfdff', marginTop: 10 };
const labelStyle = { display: 'grid', gap: 5, fontSize: 12, fontWeight: 800, color: '#25344d' };
const inputStyle = { height: 34, border: '1px solid #d7dfec', borderRadius: 9, padding: '0 10px', fontSize: 13, background: '#fff' };
const rowStyle = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 };
const helpStyle = { fontSize: 12, color: '#667085', lineHeight: 1.45, marginTop: 8 };
...
export default function HiddenEventEditor({ value, onChange }) {
  ...
  const emit = (patch) => onChange && onChange(normalizeHiddenEventParams({ ...cfg, ...patch }));
  const emitStep = (key, raw) => {
    emit({ handicapSteps: { ...cfg.handicapSteps, [key]: Number(raw || 0) } });
  };
  ...
  return (
    <div style={boxStyle}>
      ...
      <label style={labelStyle}>방식
        <select style={inputStyle} value={cfg.mode} onChange={(e) => emit({ mode: e.target.value })}>
          <option value="personal">개인 · 비밀 1대1 지목전</option>
          <option value="fourball">포볼 · 운영자 무작위 2인팀</option>
        </select>
      </label>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <div style={rowStyle}>
            <label style={labelStyle}>1조↔2조
              <input style={inputStyle} type="number" inputMode="numeric" value={cfg.handicapSteps['1-2']} onChange={(e) => emitStep('1-2', e.target.value)} />
            </label>
            <label style={labelStyle}>2조↔3조
              <input style={inputStyle} type="number" inputMode="numeric" value={cfg.handicapSteps['2-3']} onChange={(e) => emitStep('2-3', e.target.value)} />
            </label>
            <label style={labelStyle}>3조↔4조
              <input style={inputStyle} type="number" inputMode="numeric" value={cfg.handicapSteps['3-4']} onChange={(e) => emitStep('3-4', e.target.value)} />
            </label>
          </div>
          <div style={helpStyle}>
