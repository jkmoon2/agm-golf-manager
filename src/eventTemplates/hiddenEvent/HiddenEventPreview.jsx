  ...
  if (cfg.mode === 'fourball') {
    ...
    return (
      <div style={{ marginTop: 8 }}>
        <div style={{ fontSize: 12, color: '#667085', marginBottom: 8 }}>포볼팀 상태: <b>{cfg.revealed ? '공개' : '비공개'}</b></div>
        <ol style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 6 }}>
          {!rows.length && <li style={{ color: '#999', fontSize: 13 }}>아직 포볼팀이 배정되지 않았습니다.</li>}
          {rows.map((row, idx) => (
            <li key={row.key} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, border: '1px solid #e5eaf2', borderRadius: 10, padding: '8px 10px', background: '#fff' }}>
              <span><b>{idx + 1}.</b> {row.label} <small style={{ color: '#667085' }}>G합 {fmt(row.handicapSum)}</small></span>
