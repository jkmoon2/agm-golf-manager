...
function getGroupRoomBattleTypeText(type) {
  return type === 'match' ? '매치플레이' : type === 'fourball' ? '매치(포볼)' : '스트로크';
}
...
function getGroupRoomHoleBattleMetaText(params) {
  ...
  const battleTypeText = getGroupRoomBattleTypeText(safe.battleType);
  const extra = safe.mode === 'person'
    ...
    : safe.mode === 'group'
      ...
      : '';
  return `${battleTypeText} · ${modeText}${extra} · ${holeCount}홀 · ${pickCount} · ${maxCount}`;
}
